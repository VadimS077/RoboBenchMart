...
def detect_target_mask_cameras(obs_group: h5py.Group) -> List[str]:
    cameras = []
    if "sensor_data" in obs_group:
        sensor_data = obs_group["sensor_data"]
        for camera_name in sensor_data.keys():
            if "target_mask" in sensor_data[camera_name]:
                cameras.append(camera_name)
    return cameras
...
def load_trajectory_from_h5(h5_file: Path) -> Tuple[List[Dict[str, np.ndarray]], Dict[str, Any]]:
    ...
    with h5py.File(h5_file, 'r') as f:
        ...
        target_mask_cameras = (
            detect_target_mask_cameras(first_traj["obs"]) if "obs" in first_traj else []
        )
        ...
        logger.info(
            "Detected: action_dim=%s, state_dim=%s, rgb_cameras=%s, target_mask_cameras=%s",
            action_dim,
            state_dim,
            rgb_cameras,
            target_mask_cameras,
        ...
        for traj_key in traj_keys:
            ...
            if rgb_cameras and "obs" in traj:
                for camera_name in rgb_cameras:
                    ...
                    episode_data[f"rgb_{camera_name}"] = rgb[:episode_length]

            if target_mask_cameras and "obs" in traj:
                for camera_name in target_mask_cameras:
                    target_mask = traj["obs"]["sensor_data"][camera_name]["target_mask"][:]
                    episode_data[f"target_mask_{camera_name}"] = target_mask[:episode_length]
            
            if state_dim and 'obs' in traj:
                qpos = traj['obs']['agent']['qpos'][:]
                episode_data['robot_state'] = qpos[:episode_length]

            # Keep success labels from source trajectory for policy training/evaluation.
            if "success" in traj:
                success = traj["success"][:]
                episode_data["success"] = success[:episode_length].astype(np.bool_)
...
def create_video_from_frames(
    ...
    def _to_rgb_frame(frame: np.ndarray) -> np.ndarray:
        if frame.ndim == 2:
            frame = frame[..., None]
        if frame.ndim != 3:
            raise ValueError(f"Unsupported frame shape for video export: {frame.shape}")
        if frame.shape[-1] == 1:
            ch = frame[..., 0].astype(np.float32)
            max_val = float(ch.max()) if ch.size > 0 else 0.0
            # Binary masks are often uint8 in {0,1}. Scale them to full contrast.
            if max_val <= 1.0:
                ch = ch * 255.0
            ch = np.clip(ch, 0, 255).astype(np.uint8)
            frame = np.repeat(ch[..., None], 3, axis=-1)
        elif frame.shape[-1] == 3:
            if frame.dtype != np.uint8:
                frame = np.clip(frame, 0, 255).astype(np.uint8)
        else:
            raise ValueError(f"Unsupported channel count for video export: {frame.shape[-1]}")
        return frame

    target_size = (image_width, image_height)
    resized_frames = [
        resize_image_with_padding(_to_rgb_frame(frame), target_size) for frame in frames
    ]
...
def process_episode(
    ...
    if 'success' in episode_data:
        success = np.asarray(episode_data['success']).reshape(-1).astype(np.bool_)
        df_data['success'] = success[:episode_length]
    
    column_order = ['action', 'observation.state', 'success', 'timestamp', 'frame_index', 
...
def calculate_statistics(
    ...
    all_video_data_by_key: Dict[str, List[np.ndarray]],
    has_state: bool
) -> Dict[str, Any]:
    ...
    if 'success' in combined_df:
        success = combined_df['success'].astype(np.float32).values
        stats['success'] = {
            'mean': [float(success.mean())],
            'std': [float(success.std())],
            'max': [float(success.max())],
            'min': [float(success.min())],
            'count': [len(success)]
        }
    
    for video_key, episode_frames_list in all_video_data_by_key.items():
        if episode_frames_list:
            all_pixels = []
            total_frames = 0
            for episode_frames in episode_frames_list:
                arr = episode_frames.astype(np.float32)
            ...
            stats[f'observation.images.{video_key}'] = {
...
def create_meta_files(
    ...
    video_keys = list(rgb_cameras) + [f"{cam}_target_mask" for cam in target_mask_cameras]
    ...
    for ep_idx, (length, df) in enumerate(zip(episode_lengths, all_dataframes)):
        ...
        if 'success' in df:
            succ = df['success'].astype(np.float32).values
            episode_meta["stats/success/min"] = [float(succ.min())]
            episode_meta["stats/success/max"] = [float(succ.max())]
            episode_meta["stats/success/mean"] = [float(succ.mean())]
            episode_meta["stats/success/std"] = [float(succ.std())]
            episode_meta["stats/success/count"] = [length]
        
        for video_key in video_keys:
            if video_key in all_video_data_by_key and ep_idx < len(all_video_data_by_key[video_key]):
                video_data = all_video_data_by_key[video_key][ep_idx].astype(np.float32)
                if video_data.ndim == 4 and video_data.shape[-1] == 1:
                    video_data = np.repeat(video_data, 3, axis=-1)
                if video_data.max() > 1.0:
                    video_data = video_data / 255.0
                prefix = f"stats/observation.images.{video_key}"
                episode_meta[f"{prefix}/min"] = [[[float(video_data[..., i].min())]] for i in range(3)]
                episode_meta[f"{prefix}/max"] = [[[float(video_data[..., i].max())]] for i in range(3)]
                episode_meta[f"{prefix}/mean"] = [[[float(video_data[..., i].mean())]] for i in range(3)]
                episode_meta[f"{prefix}/std"] = [[[float(video_data[..., i].std())]] for i in range(3)]
    ...
    features = {
        ...
        "success": {"dtype": "bool", "shape": [1], "names": None, "fps": float(fps)},
    }
    ...
    for camera_name in target_mask_cameras:
        key = f"{camera_name}_target_mask"
    ...
    info_data = {
        ...
        "total_tasks": 1,
        "total_videos": len(episode_lengths) * (len(rgb_cameras) + len(target_mask_cameras)),
...
def main(args: Args):
    ...
    try:
        ...
        video_keys = list(info["rgb_cameras"]) + [
            f"{cam}_target_mask" for cam in info.get("target_mask_cameras", [])
        ]
        base_path = create_directory_structure(
            args.output_dir, video_keys, len(episodes), args.chunks_size
        )
        image_width, image_height = parse_image_size(args.image_size)
        
        all_dataframes = []
        all_video_data_by_key = {key: [] for key in video_keys}
        episode_lengths = []
        global_index = 0
        ...
        for episode_idx, episode_data in enumerate(tqdm(episodes, desc="Processing episodes")):
            ...
            for camera_name in info['rgb_cameras']:
                ...
                if rgb_key in episode_data:
                    all_video_data_by_key[camera_name].append(episode_data[rgb_key])
            for camera_name in info.get("target_mask_cameras", []):
                mask_key = f"target_mask_{camera_name}"
                video_key = f"{camera_name}_target_mask"
                if mask_key in episode_data:
                    all_video_data_by_key[video_key].append(episode_data[mask_key])
            ...
            for camera_name in info.get("target_mask_cameras", []):
                ...
                if mask_key in episode_data:
                    ...
                    create_video_from_frames(
                        episode_data[mask_key], video_path, args.fps, image_width, image_height
                    )
        ...
        stats = calculate_statistics(
            all_dataframes, all_video_data_by_key, info["state_dim"] is not None
        )
        ...
        create_meta_files(
            ...
            info['action_dim'], info['state_dim'], info['rgb_cameras'], info.get("target_mask_cameras", []),
            info['metadata'], task_name, args.chunks_size, args.fps,
            image_width, image_height, all_dataframes, all_video_data_by_key,
